...
static void autotune_attitude_control()
{
    ...
    case AUTOTUNE_STEP_WAITING_FOR_LEVEL:
        ...
        if ((labs(ahrs.roll_sensor) > AUTOTUNE_LEVEL_ANGLE_CD) ||
                ...
                ((ToDeg(ahrs.get_ins().get_gyro(2).x) * 100.0f) > AUTOTUNE_LEVEL_RATE_RP_CD) ||
                ((ToDeg(ahrs.get_ins().get_gyro(2).y) * 100.0f) > AUTOTUNE_LEVEL_RATE_RP_CD) ||
                ((ToDeg(ahrs.get_ins().get_gyro(2).z) * 100.0f) > AUTOTUNE_LEVEL_RATE_Y_CD) ) {
        ...
        case AUTOTUNE_AXIS_ROLL:
            ...
            autotune_start_rate = ToDeg(ahrs.get_ins().get_gyro(2).x) * 100.0f;
        ...
        case AUTOTUNE_AXIS_PITCH:
            ...
            autotune_start_rate = ToDeg(ahrs.get_ins().get_gyro(2).y) * 100.0f;
        ...
        case AUTOTUNE_AXIS_YAW:
            ...
            autotune_start_rate = ToDeg(ahrs.get_ins().get_gyro(2).z) * 100.0f;
    ...
    case AUTOTUNE_STEP_TWITCHING:
        ...
        case AUTOTUNE_AXIS_ROLL:
            if ((autotune_state.tune_type == AUTOTUNE_TYPE_SP_DOWN) || (autotune_state.tune_type == AUTOTUNE_TYPE_SP_UP)) {
                rotation_rate = rotation_rate_filt.apply(direction_sign * (ToDeg(ahrs.get_ins().get_gyro(2).x) * 100.0f), MAIN_LOOP_SECONDS);
            } else {
                rotation_rate = rotation_rate_filt.apply(direction_sign * (ToDeg(ahrs.get_ins().get_gyro(2).x) * 100.0f - autotune_start_rate), MAIN_LOOP_SECONDS);
            }
            lean_angle = direction_sign * (ahrs.roll_sensor - (int32_t)autotune_start_angle);
            break;
        case AUTOTUNE_AXIS_PITCH:
            if ((autotune_state.tune_type == AUTOTUNE_TYPE_SP_DOWN) || (autotune_state.tune_type == AUTOTUNE_TYPE_SP_UP)) {
                rotation_rate = rotation_rate_filt.apply(direction_sign * (ToDeg(ahrs.get_ins().get_gyro(2).y) * 100.0f), MAIN_LOOP_SECONDS);
            } else {
                rotation_rate = rotation_rate_filt.apply(direction_sign * (ToDeg(ahrs.get_ins().get_gyro(2).y) * 100.0f - autotune_start_rate), MAIN_LOOP_SECONDS);
            }
            lean_angle = direction_sign * (ahrs.pitch_sensor - (int32_t)autotune_start_angle);
            break;
        case AUTOTUNE_AXIS_YAW:
            if ((autotune_state.tune_type == AUTOTUNE_TYPE_SP_DOWN) || (autotune_state.tune_type == AUTOTUNE_TYPE_SP_UP)) {
                rotation_rate = rotation_rate_filt.apply(direction_sign * (ToDeg(ahrs.get_ins().get_gyro(2).z) * 100.0f), MAIN_LOOP_SECONDS);
            } else {
                rotation_rate = rotation_rate_filt.apply(direction_sign * (ToDeg(ahrs.get_ins().get_gyro(2).z) * 100.0f - autotune_start_rate), MAIN_LOOP_SECONDS);
            }
