						       ...
						       uint8_t target_system, uint8_t target_component, uint16_t seq, uint8_t frame, uint16_t command, uint8_t current, uint8_t autocontinue, float param1, float param2, float param3, float param4, int32_t x, int32_t y, float z)
{
	...
	_mav_put_uint8_t(buf, 36, autocontinue);
		...
		mav_put_int32_t_c2000(&(msg->payload64[0]), 16, x);
		mav_put_int32_t_c2000(&(msg->payload64[0]), 20, y);
		mav_put_float_c2000(&(msg->payload64[0]), 24, z);
		mav_put_uint16_t_c2000(&(msg->payload64[0]), 28, seq);
		mav_put_uint16_t_c2000(&(msg->payload64[0]), 30, command);
		mav_put_uint8_t_c2000(&(msg->payload64[0]), 32, target_system);
		mav_put_uint8_t_c2000(&(msg->payload64[0]), 33, target_component);
		mav_put_uint8_t_c2000(&(msg->payload64[0]), 34, frame);
		mav_put_uint8_t_c2000(&(msg->payload64[0]), 35, current);
		mav_put_uint8_t_c2000(&(msg->payload64[0]), 36, autocontinue);
