...
void AC_AttitudeControl::relax_bf_rate_controller()
{
    ...
    const Vector3f& gyro = _ahrs.get_ins().get_gyro(2);
...
void AC_AttitudeControl::integrate_bf_rate_error_to_angle_errors()
{
    ...
    _angle_bf_error.x += (_rate_bf_desired.x - (_ahrs.get_ins().get_gyro(2).x * AC_ATTITUDE_CONTROL_DEGX100)) * _dt;
    ...
    _angle_bf_error.y += (_rate_bf_desired.y - (_ahrs.get_ins().get_gyro(2).y * AC_ATTITUDE_CONTROL_DEGX100)) * _dt;
    ...
    _angle_bf_error.z += (_rate_bf_desired.z - (_ahrs.get_ins().get_gyro(2).z * AC_ATTITUDE_CONTROL_DEGX100)) * _dt;
...
void AC_AttitudeControl::update_rate_bf_targets()
{
	...
	_rate_bf_target.x += _angle_bf_error.y * _ahrs.get_ins().get_gyro(2).z;
	_rate_bf_target.y += -_angle_bf_error.x * _ahrs.get_ins().get_gyro(2).z;
}
...
float AC_AttitudeControl::rate_bf_to_motor_roll(float rate_target_cds)
{
    ...
    current_rate = (_ahrs.get_ins().get_gyro(2).x * AC_ATTITUDE_CONTROL_DEGX100);
...
float AC_AttitudeControl::rate_bf_to_motor_pitch(float rate_target_cds)
{
    ...
    // get current rate
    // To-Do: make getting gyro rates more efficient?
    current_rate = (_ahrs.get_ins().get_gyro(2).y * AC_ATTITUDE_CONTROL_DEGX100);
...
float AC_AttitudeControl::rate_bf_to_motor_yaw(float rate_target_cds)
{
    ...
    // get current rate
    // To-Do: make getting gyro rates more efficient?
    current_rate = (_ahrs.get_ins().get_gyro(2).z * AC_ATTITUDE_CONTROL_DEGX100);
