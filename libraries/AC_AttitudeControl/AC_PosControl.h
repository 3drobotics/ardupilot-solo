...
#define POSCONTROL_JERK_LIMIT_CMSSS             4000.0f // jerk limit on horizontal acceleration (unit: m/s/s/s)
#define POSCONTROL_ACCEL_FILTER_HZ              2.0f    // low-pass filter on acceleration (unit: hz)
#define POSCONTROL_JERK_RATIO                   3.0f    // Defines the time it takes to reach the requested acceleration
...
public:
    ...
    void set_alt_target_from_climb_rate_ff(float climb_rate_cms, float dt, bool force_descend = false);
    ...
    void set_target_to_stopping_point_z_ff();
